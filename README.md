...
## 화면별 주요 시나리오
- **모델 등록(화면1)**: 모델명과 마스터 이미지를 등록하고, 하위에 기종 코드를 추가합니다.
- **카메라 등록(화면2)**:
  - 카메라 번호, 카메라명, 시리얼 정보를 포함해 최대 32개의 카메라를 등록합니다.
  - 등록된 카메라에서 영상을 취득하고 최대 24개의 검사 영역을 할당합니다.
  - 검사 영역마다 밝기/길이 측정 등 검사 로직을 설정할 수 있습니다.
- **검사 등록(화면3)**: 모델과 기종을 선택한 뒤 카메라를 할당하고, 검사 적용 여부 및 검출 시 판정(양품/불량) 변경 처리 옵션을 설정합니다.
- **검사(메인 화면)**:
  - 모델과 기종을 설정하면 기종에 연결된 카메라에서 트리거로 영상을 취득합니다.
  - 등록된 검사 정보에 따라 카메라별 검사를 수행하고 양/불 판정을 화면에 표시하며 로그로 기록합니다.

## 개발 환경
프로젝트는 WinForms 기반으로 `net8.0-windows` 및 `win-x64` Runtime Identifier를 대상으로 합니다.
